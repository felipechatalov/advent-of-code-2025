...
fn connect(a: usize, b: usize, arr: &mut Vec<Vec<usize>>) -> bool {
    for conn in &mut *arr {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut last_connected: Pair = Pair {
        src: 0,
        dst: 0,
        dist: 0.0,
    };
    for pair in distances {
        if connect(pair.src, pair.dst, &mut connections) {
            last_connected = pair;
        }
    }

    println!("last_connected {:?}", last_connected);
    println!("{:?}", boxes[last_connected.src]);
    println!("{:?}", boxes[last_connected.dst]);
    println!(
        "{}",
        boxes[last_connected.src][0] * boxes[last_connected.dst][0]
    );
    Ok(())
}
