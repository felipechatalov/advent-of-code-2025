...
fn remove_roll(diagram: &mut Vec<Vec<char>>, i: usize, j: usize) -> () {
    diagram[i][j] = 'x';
}

fn get_rolls(diagram: &mut Vec<Vec<char>>) -> usize {
    ...
    while removed > 0 {
        ...
        for i in 0..diagram.len() {
            for j in 0..diagram[0].len() {
                ...
                {
                    // print!("was {}, ", diagram[i][j]);
                    remove_roll(diagram, i, j);
                    // print!("now {}\n", diagram[i][j]);
                    removed += 1;
                }
            }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let rolls_acessible = get_rolls(&mut diagram);
